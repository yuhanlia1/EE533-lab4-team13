...
for i in {0..3}; do
    ...
    sshpass -p $PASS ssh -o StrictHostKeyChecking=no node1@$SERVER_HOST \
		"pkill -u node1 iperf; nohup iperf -s -p $SERVER_PORT" \
		 > ./log/router/iperf_server_${SERVER_NODE}.log 2>&1 &
	sleep 1
    ...
    for j in {0..3}; do
        if [ $j -ne $i ]; then
            ...
            sshpass -p $PASS ssh -o StrictHostKeyChecking=no node1@$CLIENT_HOST \
				...
				> ./log/router/iperf_client_${CLIENT_NODE}_to_${SERVER_NODE}.log 2>&1 &
        fi
    done
done
