...
for i in {0..3}; do
    ...
    sshpass -p $PASS ssh -o StrictHostKeyChecking=no node1@$SERVER_HOST \
		"pkill -u node1 iperf; iperf -s -u -p $SERVER_PORT -i $TIME" \
		 > ./log/lab3_test/iperf_server_${SERVER_NODE}.log 2>&1 &
	sleep 1
    ...
    for j in {0..3}; do
        if [ $j -ne $i ]; then
            ...
            sshpass -p $PASS ssh -o StrictHostKeyChecking=no node1@$CLIENT_HOST \
                "iperf -c $SERVER_IP -u -b $BW -l $PKT -t $TIME -p $SERVER_PORT -i $TIME" \
				> ./log/lab3_test/iperf_client_${CLIENT_NODE}_to_${SERVER_NODE}.log 2>&1 &
        fi
    done
done
